...
if ( ! function_exists( 'sargiasrilanka_scripts' ) ) {
	/**
	 * Load theme's JavaScript and CSS sources.
	 */
	function sargiasrilanka_scripts() {
		// Get the theme data.
		...
		wp_enqueue_style( 'sargiasrilanka-styles', get_template_directory_uri() . $theme_styles, array(), $css_version );

		// Fix that the offcanvas close icon is hidden behind the admin bar.
		if ( 'bootstrap4' !== $bootstrap_version && is_admin_bar_showing() ) {
			sargiasrilanka_offcanvas_admin_bar_inline_styles();
		}

		wp_enqueue_script( 'jquery' );

		$js_version = $theme_version . '.' . filemtime( get_template_directory() . $theme_scripts ); // @phpstan-ignore-line -- file exists
		wp_enqueue_script( 'sargiasrilanka-scripts', get_template_directory_uri() . $theme_scripts, array(), $js_version, true );
		if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
			wp_enqueue_script( 'comment-reply' );
		}
	}
} // End of if function_exists( 'sargiasrilanka_scripts' ).

add_action( 'wp_enqueue_scripts', 'sargiasrilanka_scripts' );

if ( ! function_exists( 'sargiasrilanka_offcanvas_admin_bar_inline_styles' ) ) {
	/**
	 ...
	 * @since 1.2.0
	 */
	function sargiasrilanka_offcanvas_admin_bar_inline_styles() {
		$navbar_type = get_theme_mod( 'sargiasrilanka_navbar_type', 'collapse' );
		...
		wp_add_inline_style( 'sargiasrilanka-styles', $css );
	}
}
